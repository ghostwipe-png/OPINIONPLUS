...
stories.post('/', requireAuth, async (c) => {
  ...
  await c.env.DB.prepare('INSERT INTO stories (id, author_id, title, excerpt, body, type, privacy, cover_image) VALUES (?, ?, ?, ?, ?, ?, ?, ?)')
    .bind(id, user.id, body.title, body.excerpt || '', body.body, body.type || 'story', body.privacy || 'public', body.coverImage || null).run();
...
stories.patch('/:id', requireAuth, async (c) => {
  ...
  const body = await c.req.json();
  await c.env.DB.prepare("UPDATE stories SET title = ?, excerpt = ?, body = ?, type = ?, privacy = ?, cover_image = ?, updated_at = datetime('now') WHERE id = ?")
    .bind(body.title ?? existing.title, body.excerpt ?? existing.excerpt, body.body ?? existing.body, body.type ?? existing.type, body.privacy ?? existing.privacy, body.coverImage ?? existing.cover_image, id).run();
...
// ---------------------------------------------------------------------------
// Deep Search — searches title, excerpt, AND body across all content types
// ---------------------------------------------------------------------------
