...
// GET /keys — list all API keys for the logged-in user
...
keys.post('/', requireAuth, async (c) => {
  ...
  const raw = `op_${crypto.randomUUID().replace(/-/g, '')}`;
  ...
  return c.json({ id, name: name.trim(), key: raw, prefix, created_at: new Date().toISOString() }, 201);
});
