...
async function activateProSubscription(db, userId, planCode, subscriptionCode) {
  await db
    .prepare(
      `INSERT INTO api_usage (user_id, tier, subscription_active, plan_code, subscription_code)
       VALUES (?, 'pro', 1, ?, ?)
...
payments.post('/initialize', requireAuth, async (c) => {
  ...
  const requestBody = {
    ...
    metadata: { user_id: user.id, transaction_id: txnId, credits: pkg.credits, package: pkg.name, type: 'sms_credits' },
  };
  ...
  try {
    const response = await fetch('https://api.paystack.co/transaction/initialize', {
      ...
      body: JSON.stringify({
        ...
        plan: PRO_PLAN_CODE,
        currency: 'KES',
        metadata: { user_id: user.id, type: 'api_pro_subscription' },
      }),
    });
